...
class BaiduChecker extends BaseChecker
{
    ...
    private function verifyPassCode($shareURL, $shorturl, $password)
    {
        ...
        error_log('BaiduChecker: verifyPassCode URL=' . $apiURL . ', shareURL=' . $shareURL . ', cookies=' . $cookieStr);
        ...
        if ($error) {
            error_log('BaiduChecker: verifyPassCode curl error: ' . $error);
            return null;
        }

        $result = json_decode($response, true);
        if (!$result) {
            error_log('BaiduChecker: verifyPassCode JSON decode failed, response: ' . substr($response, 0, 500));
            return null;
        }

        $errno = isset($result['errno']) ? $result['errno'] : null;
        error_log('BaiduChecker: verifyPassCode errno=' . $errno . ', response: ' . json_encode($result, JSON_UNESCAPED_UNICODE));
        if ($errno !== 0 && $errno !== '0') {
            return null;
        }
