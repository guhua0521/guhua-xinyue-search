...
class AlipanPan extends BasePan
{
    ...
    public function getFiles($pdir_fid=0)
    {
        ...
        $tokenInfo = $this->manageAccessToken($tokenFile);
        if(empty($tokenInfo['access_token'])){
            return jerr2('登录状态异常，获取access_token失败');
        }
        $access_token = $tokenInfo['access_token'];
        $drive_id = $tokenInfo['drive_id'];
    ...
    public function transfer($share_id)
    {
        ...
        if($this->isType == 1){
            ...
            // 获取 share_token 用于资源验证（与夸克网盘保持一致）
            $res = $this->getAlipan2($share_id);
            if(isset($res['share_token'])){
                $urls['stoken'] = $res['share_token'];
            }
        ...
        foreach ($file_infos as $key=>$value) {
            ...
            $data3['requests'][$key]['body']['to_drive_id'] = $drive_id;
     ...
     * @return array 包含 access_token 和 drive_id 的数组
     */
        ...
        if (file_exists($tokenFile)) {
            ...
            if (isset($tokenData['access_token']) 
                ...
                return [
                    ...
                    'drive_id' => $tokenData['drive_id']
                ];
            }
        }
        
        // 获取新的token
        $tokenInfo = $this->getNewAccessToken();
    ...
    private function getNewAccessToken()
    {
        ...
        // 尝试从 token 响应中获取 resource_drive_id
        $drive_id = $res['resource_drive_id'] ?? '';
        
        // 如果没有 resource_drive_id，则调用 list_my_drives 接口获取
        if (empty($drive_id) && !empty($access_token)) {
            $drive_id = $this->getResourceDriveId($access_token);
        }
        
        // 如果还是没有，则使用 default_drive_id 作为后备
        if (empty($drive_id)) {
            $drive_id = $res['default_drive_id'] ?? $res['drive_id'] ?? '';
        }
     ...
     * @return string
     */
    private function getResourceDriveId($access_token)
    {
        $urlHeader = [
            'Accept: application/json, text/plain, */*',
        ...
        $res = curlHelper("https://api.aliyundrive.com/v2/drive/list_my_drives", "POST", json_encode($listData), $urlHeader)['body'];
        $res = json_decode($res, true);
        
        if (isset($res['items']) && is_array($res['items'])) {
            foreach ($res['items'] as $drive) {
                // 查找 drive_name 为 'resource' 或 category 为 'resource' 的盘
                if ((isset($drive['drive_name']) && $drive['drive_name'] === 'resource') ||
                    (isset($drive['category']) && $drive['category'] === 'resource')) {
    ...
    public function deletepdirFid($filelist)
    {
        ...
        foreach ($filelist as $key=>$value) {
            ...
            $urlData['requests'][$key]['body']['drive_id'] = $drive_id;
