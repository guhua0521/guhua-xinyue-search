...
class Conf extends QfShop
{
    ...
    public function getBaseConfig()
    {
        ...
        // 分站管理员：读取分站自己的基础配置
        if (!\app\service\SiteService::isSuperAdmin($this->admin)) {
            $siteId = $this->admin['site_id'];
            $siteConf = (new \app\model\SiteConf())->getSiteConf($siteId);
            $datalist = $this->model
                ->where('conf_status', 1)
                ->whereIn('conf_key', \app\service\SiteService::$editableKeys)
                ->order("conf_sort desc, " . $this->pk . " asc")
                ->select();
            foreach ($datalist as $value) {
                $key = $value['conf_key'];
                $value['conf_value'] = $siteConf[$key] ?? $value['conf_value'];
    ...
    public function updateBaseConfig()
    {
        ...
        if (!\app\service\SiteService::isSuperAdmin($this->admin)) {
            $siteId = $this->admin['site_id'];
            $siteConfModel = new \app\model\SiteConf();
            $editable = array_flip(\app\service\SiteService::$editableKeys);
            foreach (input("post.") as $k => $v) {
                ...
                $siteConfModel->saveConf($siteId, $k, $v);
            }
            return jok("配置修改成功");
        }
        foreach (input("post.") as $k => $v) {
