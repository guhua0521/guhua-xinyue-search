...
class Admin extends QfShop
{
    ...
    public function login()
    {
        ...
        if ($admin) {
            ...
            if ($access) {
                // 按账号所属站点写入独立登录Cookie，支持多个后台同时登录
                $siteKey = '';
                $site = null;
                if (intval($admin['site_id']) > 0) {
                    $site = \think\facade\Db::name('site')->where('site_id', intval($admin['site_id']))->find();
                    $siteKey = $site ? (string) $site['site_key'] : '';
                }
                $cookieName = \app\service\SiteService::adminCookieName($siteKey);
                setcookie($cookieName, $access['access_token'], time() + 3600, '/');
                if ($siteKey) {
                    setcookie('site_key', $siteKey, time() + 86400 * 7, '/');
                } else {
                    setcookie('site_key', '', time() - 3600, '/');
                }
                $redirect = $siteKey ? '/qfadmin?site=' . $siteKey : '/qfadmin';
                return jok('登录成功', [
    ...
    public function logout()
    {
        ...
        $siteKey = '';
        if ($access_token) {
            $access = $this->accessModel->where('access_token', $access_token)->find();
            if ($access) {
                $admin = $this->model->where('admin_id', $access['access_admin'])->find();
                if ($admin && intval($admin['site_id']) > 0) {
                    $site = \think\facade\Db::name('site')->where('site_id', intval($admin['site_id']))->find();
                    $siteKey = $site ? (string) $site['site_key'] : '';
                }
                $cookieName = \app\service\SiteService::adminCookieName($siteKey);
